...
rex_extension::register('OUTPUT_FILTER', array('rex_extension_log', 'doLog'));
 ...
 * @author staabm
 */
class rex_extension_log extends rex_extension
{
    ...
    foreach(self::$log as $count=>$entry)
    {
      ...
      else
      {
        $log_table[] = array(
          $i,                                                        // #
          $entry['type'],                                            // Type
          $entry['ep'] /*. ($entry['read_only'] ? ' (readonly)' : '')*/, // ExtensionPoint
   ...
   * @return string
   **/
  static private function callableAsString($mixed)
  {
