...
if(isset($user) && is_a($user,'rex_user'))
{
  if($user->isAdmin())
  {
    ...
    if($this->getConfig('sql_log')==1) {
      rex_sql::setFactoryClass('rex_sql_log');
    }

    if($this->getConfig('ep_log')==1) {
      rex_extension::setFactoryClass('rex_extension_log');
    }

    if($this->getConfig('api_log')==1) {
      rex_api_function::setFactoryClass('rex_api_function_log');
    }

    rex_logger::setFactoryClass('rex_logger_log');
  }
}
else
{
